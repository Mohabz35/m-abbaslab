...
async function logJarvisInteraction(sender, senderName, messageText, reply) {
  ...
  try {
    ...
    console.warn('[M-JARVIS] Failed to log jarvis interaction:', error.message)
  }
}
...
function scheduleReconnect(reason, options = {}) {
  ...
  let delay = Math.min(60000, 5000 * (2 ** Math.max(reconnectAttempts - 1, 0)))
  ...
  reconnectTimer = setTimeout(async () => {
    ...
    if (restoreBackup || options.restoreBackup) {
  ...
  console.log(`[M-JARVIS] Reconnect scheduled in ${delay}ms. Reason: ${reason}`)
}

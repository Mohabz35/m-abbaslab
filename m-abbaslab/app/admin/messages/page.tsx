    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          <button onClick={() => setSelectedSubmission(null)} className="p-2 hover:bg-gray-100 dark:bg-gray-800 rounded-lg">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="text-gray-700 dark:text-gray-200 leading-relaxed whitespace-pre-wrap">{selectedSubmission.message}</div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
              ...
              <textarea
                ...
                className="w-full border border-gray-200 dark:border-gray-700 rounded-lg p-3 text-sm resize-none"
              />
              <button
                onClick={() => saveNotes(selectedSubmission.id)}
                className="mt-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg text-sm font-medium"
              >
                Save Notes
              </button>
            </div>
          ...
          <div className="space-y-4">
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
              ...
              <dl className="space-y-3 text-sm">
                <div className="flex justify-between"><dt className="text-gray-500 dark:text-gray-400">Subject</dt><dd className="font-medium">{selectedSubmission.subject || 'N/A'}</dd></div>
                <div className="flex justify-between"><dt className="text-gray-500 dark:text-gray-400">Type</dt><dd className="font-medium capitalize">{selectedSubmission.inquiry_type}</dd></div>
                <div className="flex justify-between"><dt className="text-gray-500 dark:text-gray-400">Date</dt><dd className="font-medium">{new Date(selectedSubmission.created_at).toLocaleString()}</dd></div>
                <div className="flex justify-between"><dt className="text-gray-500 dark:text-gray-400">Status</dt><dd><span className={`px-2 py-1 rounded-full text-xs font-medium border border-gray-200 dark:border-gray-700 ${statusColors[selectedSubmission.status]}`}>{statusLabels[selectedSubmission.status]}</span></dd></div>
              </dl>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
              ...
              <div className="space-y-2">
                <button onClick={() => updateStatus(selectedSubmission.id, 'in_progress')} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:bg-gray-800/50 rounded-lg flex items-center gap-2">
                  <Clock className="w-4 h-4 text-amber-500" /> Mark In Progress
                </button>
                <button onClick={() => updateStatus(selectedSubmission.id, 'replied')} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:bg-gray-800/50 rounded-lg flex items-center gap-2">
                  <CheckCircle className="w-4 h-4 text-green-500" /> Mark Replied
                </button>
                <button onClick={() => updateStatus(selectedSubmission.id, 'archived')} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:bg-gray-800/50 rounded-lg flex items-center gap-2">
                  <Archive className="w-4 h-4 text-gray-500 dark:text-gray-400" /> Archive
                </button>
                <button onClick={() => updateStatus(selectedSubmission.id, 'spam')} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:bg-gray-800/50 rounded-lg flex items-center gap-2">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">{submissions.length} total submissions</p>
        </div>
        <button onClick={exportCSV} className="px-4 py-2 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg text-sm font-medium flex items-center gap-2">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-3 border border-gray-200 dark:border-gray-700 rounded-xl text-sm"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
          <Mail className="w-12 h-12 text-gray-400 dark:text-gray-600 dark:text-gray-300 mx-auto mb-3" />
          ...
          {filtered.map(s => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300 truncate">{s.message}</p>
              </div>
              <div className="text-right flex-shrink-0">
                <span className={`inline-block px-2 py-1 rounded-full text-[10px] font-medium border border-gray-200 dark:border-gray-700 ${statusColors[s.status]}`}>
                  {statusLabels[s.status]}
                </span>
                <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">{new Date(s.created_at).toLocaleDateString()}</div>
              </div>
            </div>
          ))}
        </div>

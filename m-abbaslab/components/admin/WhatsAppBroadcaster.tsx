...
export default function WhatsAppBroadcaster() {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 space-y-5">
      ...
      <div>
        <label className="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-400 dark:text-gray-600 dark:text-gray-300">Message</label>
        <textarea
          value={message}
        ...
        <div className="mt-2 flex items-center justify-between">
          <p className="text-xs text-gray-500 dark:text-gray-400">{message.length} characters</p>
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {QUICK_TEMPLATES.map((template, index) => (
            <button
              ...
              title={template}
              className="text-xs px-2.5 py-1.5 rounded-full border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 dark:text-gray-600 dark:text-gray-300 hover:border-green-400 hover:text-green-600 transition-colors"
            >
              {`Template ${index + 1}`}
            </button>
          ))}
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        <input
          ...
          className="w-4 h-4"
        />
        <span className="text-sm font-medium text-gray-700 dark:text-gray-400 dark:text-gray-600 dark:text-gray-300">
      ...
      {!sendToAll && (
        <div>
          <label className="block text-sm font-bold mb-2 text-gray-700 dark:text-gray-400 dark:text-gray-600 dark:text-gray-300">
            ...
            {subscribers.map((subscriber) => (
              <label key={subscriber.id} className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4"
                />
                <span className="text-sm text-gray-700 dark:text-gray-400 dark:text-gray-600 dark:text-gray-300">
                  {subscriber.name || 'Unnamed'} ({subscriber.phone_number})
                </span>
              </label>
            ))}
            {subscribers.length === 0 && (
              <p className="text-xs text-gray-500 dark:text-gray-400">No active subscribers found.</p>
            )}
          </div>
        </div>
      )}
      ...
      <div className="pt-2 border-t border-gray-100 dark:border-gray-800">
        <h3 className="text-sm font-bold mb-3 flex items-center gap-2 text-gray-700 dark:text-gray-400 dark:text-gray-600 dark:text-gray-300">
        ...
        <div className="space-y-2">
          {history.slice(0, 5).map((broadcast) => (
            ...
            >
              <p className="text-sm text-gray-700 dark:text-gray-400 dark:text-gray-600 dark:text-gray-300 line-clamp-2">{broadcast.message}</p>
          ...
          {history.length === 0 && <p className="text-xs text-gray-500 dark:text-gray-400">No broadcasts yet.</p>}
        </div>
      </div>
    </div>
  )

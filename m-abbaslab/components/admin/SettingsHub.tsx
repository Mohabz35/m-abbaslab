...
export default function SettingsHub() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tabs */}
      <div className="flex flex-wrap gap-2 border-b border-gray-200 dark:border-gray-700 border-slate-800 pb-4">
        {tabs.map(tab => {
          const Icon = tab.icon
      ...
      {/* Tab Content */}
      {activeTab === 'system' && (
        <div className="bg-slate-900 border border-gray-200 dark:border-gray-700 border-slate-800 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-8 border-b border-gray-200 dark:border-gray-700 border-slate-800 pb-6">
            <div>
          ...
          {sysLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-slate-950 p-5 rounded-xl border border-gray-200 dark:border-gray-700 border-slate-800">
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                      <div className="px-5 pb-5 space-y-4 border-t border-slate-700/50">
                        {mod.settings.map(setting => (
                          <div key={setting.key} className="flex items-center justify-between py-3 border-b border-gray-200 dark:border-gray-700 border-slate-700/30 last:border-0">
                            ...
                            <div className="shrink-0">
                              {setting.type === 'toggle' ? (
                                ...
                                >
                                  <div className={`absolute top-0.5 w-5 h-5 bg-white dark:bg-gray-800 rounded-full shadow transition-transform ${setting.value ? 'translate-x-6' : 'translate-x-0.5'}`} />
                                </button>
                              ) : setting.type === 'select' ? (
                                <select value={setting.value} onChange={e => updateSetting(mod.id, setting.key, e.target.value)} className="bg-slate-900 border border-gray-200 dark:border-gray-700 border-slate-600 rounded-lg px-3 py-2 text-sm text-white outline-none">
                                ...
                                <input
                                  ...
                                  className="bg-slate-900 border border-gray-200 dark:border-gray-700 border-slate-600 rounded-lg px-3 py-2 text-sm text-white w-24 text-right outline-none"
                                />
                              )}
                            </div>
                          </div>
          ...
          <div className="bg-slate-900 border border-gray-200 dark:border-gray-700 border-slate-800 rounded-2xl p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={displayName} onChange={e => setDisplayName(e.target.value)} className="w-full bg-slate-950 border border-gray-200 dark:border-gray-700 border-slate-800 rounded-lg px-4 py-2 text-white focus:border-cyan-500 outline-none" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">System Biography</label>
                <textarea value={bio} onChange={e => setBio(e.target.value)} rows={4} className="w-full bg-slate-950 border border-gray-200 dark:border-gray-700 border-slate-800 rounded-lg px-4 py-2 text-white focus:border-cyan-500 outline-none resize-none" />
              </div>
          ...
          <div className="bg-slate-900 border border-gray-200 dark:border-gray-700 border-slate-800 rounded-2xl p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} className="w-full bg-slate-950 border border-gray-200 dark:border-gray-700 border-slate-800 rounded-lg px-4 py-2 text-white focus:border-amber-500 outline-none" />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-2">Confirm New Password</label>
                <input type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="w-full bg-slate-950 border border-gray-200 dark:border-gray-700 border-slate-800 rounded-lg px-4 py-2 text-white focus:border-amber-500 outline-none" />
              </div>
        ...
        <div className="bg-slate-900 border border-gray-200 dark:border-gray-700 border-slate-800 rounded-2xl p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-700 border-slate-800 text-slate-400 text-xs uppercase tracking-wider">
